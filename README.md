...
每次调查的事件流和当前状态都会写入 SQLite。浏览器刷新或后端重启后，检索历史仍可恢复并继续回看；新调查不会覆盖旧调查。
...
公众输入过滤目前可以拦截部分提示词注入、虚构前提和明显跑题请求，但对经过措辞变化的恶意提问、角色扮演式请求、历史词汇与虚构内容混合的问题仍存在漏判。现有测试中，“如何成为神里绫华的狗”被错误放行并创建了调查任务，说明当前基于规则的范围判断仍需升级。下一阶段将完善通用语义分类、上下文逃逸识别、调查启动前核验和运行时熔断，确保被拒绝的问题不创建调查记录，也不触发模型与联网检索。
